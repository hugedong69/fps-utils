...
function onSpawnUser(event) {
  ...
  self.spawnedPlayers.set(longAsString(event.gameId), event);
...
function onFear(event) {
  ...
  if (
    !event.target.equals(self.myId) && config.mode === 3
  ) {
    return false;
  }
...
function FpsUtils2(dispatch) {
  ...
  const hook = (...args) => {
    const hookObj = dispatch.hook(...args);
    dataMap.set(hookObj, this);
    return hookObj;
  };
